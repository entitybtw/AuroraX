...
import { Edit3, Loader2, Plus, Save, Trash2, Trash2Icon, X } from "lucide-react";
...
export function CombosPage(): JSX.Element {
  ...
  async function bulkDelete(): Promise<void> {
    for (const name of bulk.selected) {
      const view = combos.find((v) => v.combo.name === name);
      if (view) await remove(view);
    }
    bulk.clear();
  }

  const comboIds = React.useMemo(() => combos.map((v) => v.combo.name), [combos]);
      ...
      {loading ? <Surface className="flex items-center gap-2 p-6 text-sm text-muted-foreground"><Loader2 className="h-4 w-4 animate-spin" />Loading combos...</Surface> : combos.length === 0 ? <EmptyState title="No combos configured">Create a combo to expose an ordered fallback chain as a model.</EmptyState> : (
        <>
          <BulkActionsBar count={bulk.selectedCount} onClear={bulk.clear}>
            <Button size="sm" variant="outline" onClick={() => void bulkDelete()} className="text-[11px] h-7 shrink-0 text-destructive hover:bg-destructive/10">
