...
// OpenCodeHeaderRules returns the canonical header rules the OpenCode CLI sends
...
// HeaderRuleDiff returns the canonical OpenCode header rules that are present
// in the live configuration but differ from the expected defaults. Missing
// headers are not reported — use EnsureOpenCodeRules to add those. An empty
// result means every configured OpenCode header matches the default, so the
...
func HeaderRuleDiff(existing ProviderRule) []HeaderRule {
	...
	diff := []HeaderRule{}
	for _, want := range OpenCodeHeaderRules() {
...
// EnsureOpenCodeRules merges the canonical OpenCode header rules into an
...
func EnsureOpenCodeRules(existing ProviderRule) (ProviderRule, []string, []string) {
	added := []string{}
	skipped := []string{}
