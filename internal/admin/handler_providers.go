...
func (h *Handler) buildProviderStatusResponse() providerStatusResponse {
	...
	for _, name := range names {
		...
		if h.providerOverrides != nil {
			if _, isOverride := h.providerOverrides.get(name); isOverride {
...
func parseOverrideModels(models string) []string {
	if models == "" {
		return nil
	}
...
func buildProviderStatusItem(name string, cfg providers.SanitizedProviderConfig, runtime providers.ProviderRuntimeSnapshot) providerStatusItemResponse {
	...
	configSource := ConfigSourceStatic
	if runtime.Registered && !runtime.RegistryInitialized {
		configSource = ConfigSourceEnvVar
	}
