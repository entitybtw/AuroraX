...
func (l *recordingAuditLogger) BroadcastLive(*auditlog.LogEntry) {}

func (l *recordingAuditLogger) SubscribeLive(_ int) *auditlog.LogSubscriber {
	ch := make(chan *auditlog.LogEntry, 1)
	close(ch)
	return &auditlog.LogSubscriber{ID: "recording", Entries: ch}
}

func (l *recordingAuditLogger) UnsubscribeLive(_ string) {}

func (l *recordingAuditLogger) LiveSubscriberCount() int { return 0 }
...
func TestHandleRequest_RejectsInvalidCacheOverrideHeaders(t *testing.T) {
	m := NewResponseCacheMiddlewareWithStore(cache.NewMapStore(), time.Hour)
	e := echo.New()
	body := []byte(`{"model":"gpt-4","messages":[{"role":"user","content":"hi"}]}`)
	req := httptest.NewRequest(http.MethodPost, "/v1/chat/completions", bytes.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("X-Cache-Type", "broken")
	rec := httptest.NewRecorder()
	c := e.NewContext(req, rec)
	...
	if err == nil {
		t.Fatal("HandleRequest() error = nil, want invalid request error")
	}
	if gatewayErr, ok := err.(*core.GatewayError); !ok || gatewayErr.Type != core.ErrorTypeInvalidRequest {
		t.Fatalf("HandleRequest() error = %T %#v, want invalid request gateway error", err, err)
	}
}

func TestHandleInternalRequest_RejectsInvalidCacheOverrideHeaders(t *testing.T) {
	m := NewResponseCacheMiddlewareWithStore(cache.NewMapStore(), time.Hour)
	ctx := core.WithRequestSnapshot(context.Background(), core.NewRequestSnapshot(
		...
		nil,
		nil,
		map[string][]string{"X-Cache-Semantic-Threshold": {"2"}},
		"application/json",
		nil,
		false,
		"",
	...
	_, err := m.HandleInternalRequest(ctx, http.MethodPost, "/v1/chat/completions", []byte(`{"model":"gpt-4","messages":[{"role":"user","content":"hi"}]}`), func(c *echo.Context) error {
		return c.JSON(http.StatusOK, map[string]string{"ok": "1"})
	})
	if err == nil {
		t.Fatal("HandleInternalRequest() error = nil, want invalid request error")
	...
	if gatewayErr, ok := err.(*core.GatewayError); !ok || gatewayErr.Type != core.ErrorTypeInvalidRequest {
		t.Fatalf("HandleInternalRequest() error = %T %#v, want invalid request gateway error", err, err)
	}
}
...
func TestInternalRequestHeaders_AllowlistsSafeSnapshotHeaders(t *testing.T) {
	...
	if got := headers.Get("User-Agent"); got != "aurora-test" {
		t.Fatalf("User-Agent = %q, want aurora-test", got)
	}
...
func TestInternalCacheType_ParsesHeaderShapes(t *testing.T) {
	...
	for _, tc := range cases {
		if got := parseInternalCacheType(tc.headerValue); got != tc.want {
			t.Fatalf("parseInternalCacheType(%q) = %q, want %q", tc.headerValue, got, tc.want)
		}
	}
}
...
func TestRenderCachedChatStream_EmitsStandaloneUsageChunk(t *testing.T) {
	...
	parseSSE(replay, func(event map[string]any) {
