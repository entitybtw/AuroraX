...
func (s *ttlRecordingStore) Get(context.Context, string) ([]byte, error) {
	return nil, nil
}

func (s *ttlRecordingStore) Set(_ context.Context, _ string, _ []byte, ttl time.Duration) error {
	s.lastTTL = ttl
	s.sets++
	return nil
}

func (s *ttlRecordingStore) Close() error {
	return nil
}
...
func TestSimpleCacheMiddleware_RespectsXCacheTTLOverride(t *testing.T) {
	...
	if store.lastTTL != 17*time.Second {
		t.Fatalf("store ttl = %v, want 17s", store.lastTTL)
	}
}

func TestSimpleCacheMiddleware_DebugHeadersOnExactHit(t *testing.T) {
	store := cache.NewMapStore()
	defer store.Close()
	mw := NewResponseCacheMiddlewareWithStore(store, time.Hour)
	e := echo.New()
	...
	firstReq.Header.Set("X-Cache-Debug", "true")
	...
	secondReq.Header.Set("X-Cache-Debug", "true")
	secondRec := httptest.NewRecorder()
	e.ServeHTTP(secondRec, secondReq)

	if got := secondRec.Header().Get("X-Cache"); got != "HIT (exact)" {
		t.Fatalf("X-Cache = %q, want HIT (exact)", got)
	}
	if got := secondRec.Header().Get("X-Cache-Debug"); got != "true" {
		t.Fatalf("X-Cache-Debug = %q, want true", got)
	}
	if got := secondRec.Header().Get("X-Cache-Layer"); got != "exact" {
		t.Fatalf("X-Cache-Layer = %q, want exact", got)
	}
	if got := secondRec.Header().Get("X-Cache-Exact-Key-Hash"); got == "" {
		t.Fatal("X-Cache-Exact-Key-Hash = empty, want cache key hash")
	}
}
...
func TestHashRequest_TenantChangesKey(t *testing.T) {
	body := []byte(`{"model":"gpt-4","messages":[{"role":"user","content":"hi"}]}`)
	plan := &core.Workflow{
		Mode:         core.ExecutionModeTranslated,
		ProviderType: "openai",
	...
	ctxA := scope.WithTenantID(context.Background(), "tenant-a")
	ctxB := scope.WithTenantID(context.Background(), "tenant-b")
	first := hashRequestForContext(ctxA, "/v1/chat/completions", body, plan)
	second := hashRequestForContext(ctxB, "/v1/chat/completions", body, plan)
	defaultScoped := hashRequestForContext(context.Background(), "/v1/chat/completions", body, plan)

	if first == second {
		t.Fatal("tenant id should affect exact cache key")
	}
	if first == defaultScoped || second == defaultScoped {
		t.Fatal("default tenant scope should not collide with explicit tenants")
	}
}
...
func TestRequestBodyForCache_BodyNotCapturedTakesPrecedenceOverEmptySnapshotBody(t *testing.T) {
	...
	body, cacheable, err := readRequestBody(req)
	if err != nil {
		t.Fatalf("readRequestBody() error = %v", err)
	}
	if cacheable {
		t.Fatalf("readRequestBody() cacheable = true, want false (body=%q)", body)
	}
	if body != nil {
...
func BenchmarkIsStreamingRequestGJSON(b *testing.B) {
	b.ReportAllocs()
	for b.Loop() {
		if !isStreamingGJSON("/v1/chat/completions", benchmarkStreamingBody) {
			b.Fatal("expected streaming request")
		}
	}
}
...
func TestSimpleCacheMiddleware_LimitsConcurrentCacheWrites(t *testing.T) {
	...
	const requestCount = writeWorkerCount * 2
	...
	if got := store.maxConcurrent.Load(); got > writeWorkerCount {
		t.Fatalf("expected at most %d concurrent cache writes, got %d", writeWorkerCount, got)
	}
