...
func TestSemanticCacheMiddleware_CloseFinalUserTextSharesNamespace(t *testing.T) {
	...
	body2 := []byte(`{"model":"gpt-4","messages":[{"role":"user","content":"What is 2+2 please?"}]}`)
	...
	if rec.Header().Get("X-Cache") != "HIT (semantic)" {
		t.Fatalf("near-identical last user text should share params namespace, got X-Cache=%q", rec.Header().Get("X-Cache"))
	}
}

func TestSemanticCacheMiddleware_OutputIntentIsolatesCodeRequests(t *testing.T) {
	m, store, emb := newTestSemanticMiddleware(0.90, 10, false)
	emb.vector = []float32{1, 0, 0}

	body1 := []byte(`{"model":"gpt-4","messages":[{"role":"user","content":"What is DFS?"}]}`)
	body2 := []byte(`{"model":"gpt-4","messages":[{"role":"user","content":"Give me full code for DFS."}]}`)
	...
	rec := serveSemanticRequest(t, m, body2, "")
	if rec.Header().Get("X-Cache") == "HIT (semantic)" {
		t.Fatal("code-generation request should not reuse a short explanatory semantic cache entry")
	}
}
...
func TestSemanticCacheMiddleware_MultiTurnParaphraseLastUserHit(t *testing.T) {
	...
	body2 := []byte(`{"model":"gpt-4","messages":[
		{"role":"user","content":"Remember the number 7."},
		{"role":"assistant","content":"OK."},
		{"role":"user","content":"What is 2+2 please?"}
	]}`)
	...
	rec := serveSemanticRequest(t, m, body2, "")
	if rec.Header().Get("X-Cache") != "HIT (semantic)" {
		t.Fatalf("multi-turn near-identical final user should hit, got X-Cache=%q", rec.Header().Get("X-Cache"))
	}
}
...
func TestComputeParamsHash_TenantChangesHash(t *testing.T) {
	...
	first := computeParamsHashForContext(ctxA, body, "/v1/chat/completions", plan, "", "embedder")
	second := computeParamsHashForContext(ctxB, body, "/v1/chat/completions", plan, "", "embedder")
	defaultScoped := computeParamsHashForContext(context.Background(), body, "/v1/chat/completions", plan, "", "embedder")

	if first == second {
		t.Fatal("tenant id should affect semantic params_hash")
	}
...
func TestMapVecStore_DeleteExpiredOnlyRemovesExpired(t *testing.T) {
	...
	_ = store.Insert(ctx, "key-expired", []float32{1, 0}, nil, "ph", "expired prompt", -1*time.Second)
	_ = store.Insert(ctx, "key-live", []float32{0, 1}, nil, "ph", "live prompt", time.Hour)
