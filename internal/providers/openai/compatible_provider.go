...
var debugUpstreamModel = upstreamModelDebugEnabled()

func isNotFoundError(err error) bool {
	var gwErr *core.GatewayError
	if errors.As(err, &gwErr) {
		return gwErr.HTTPStatusCode() == http.StatusNotFound
	}
	return false
}
...
type CompatibleProviderConfig struct {
	...
	ModelNameTransform func(string) string
}
...
func NewCompatibleProvider(apiKey string, opts providers.ProviderOptions, cfg CompatibleProviderConfig) *CompatibleProvider {
	p := &CompatibleProvider{
		...
		modelNameTransform: cfg.ModelNameTransform,
	}
...
func (p *CompatibleProvider) SetModelNameTransform(transform func(string) string) {
	p.modelNameTransform = transform
}

func (p *CompatibleProvider) transformModel(model string) string {
	if p.modelNameTransform != nil {
		return p.modelNameTransform(model)
	}
	return model
}
...
func (p *CompatibleProvider) ChatCompletion(ctx context.Context, req *core.ChatRequest) (*core.ChatResponse, error) {
	...
	forwardReq := *req
	forwardReq.Model = p.transformModel(req.Model)
	logForwardedModel("chat_completions", p.providerName, req.Model, forwardReq.Model)
	var resp core.ChatResponse
	body, err := chatRequestBody(&forwardReq)
	if err != nil {
		return nil, err
	}
...
func logForwardedModel(operation, providerName, requestedModel, forwardedModel string) {
	if !debugUpstreamModel {
		return
	}
	slog.Info("upstream model dispatch",
		"operation", operation,
...
func upstreamModelDebugEnabled() bool {
	value := strings.ToLower(strings.TrimSpace(os.Getenv("AURORA_DEBUG_UPSTREAM_MODEL")))
...
func (p *CompatibleProvider) StreamChatCompletion(ctx context.Context, req *core.ChatRequest) (io.ReadCloser, error) {
	...
	forwardReq := *req
	forwardReq.Model = p.transformModel(req.Model)
	streamReq := forwardReq.WithStreaming()
...
func (p *CompatibleProvider) Responses(ctx context.Context, req *core.ResponsesRequest) (*core.ResponsesResponse, error) {
	...
	if err != nil {
		// If provider returns 404, fall back to Responses via Chat adapter
...
func (p *CompatibleProvider) CountResponseInputTokens(ctx context.Context, req *core.ResponsesRequest) (*core.ResponseInputTokensResponse, error) {
	...
	err := p.Do(ctx, llmclient.Request{
		...
		Body:     responseInputTokensRequestFromResponses(&forwardReq),
	}, &resp)
	if err != nil {
		return nil, err
	}
...
func (p *CompatibleProvider) CompactResponse(ctx context.Context, req *core.ResponsesRequest) (*core.ResponseCompactResponse, error) {
	...
	err := p.Do(ctx, llmclient.Request{
		...
		Body:     responseCompactRequestFromResponses(&forwardReq),
	}, &resp)
	if err != nil {
		return nil, err
	}
...
// Rerank sends a rerank request to an OpenAI-compatible upstream that exposes
// a /rerank endpoint (Jina, Cohere, vLLM, etc.). Upstreams that don't implement
// the route will surface their own error to the caller; the gateway does not
// emulate rerank behavior.
func (p *CompatibleProvider) Rerank(ctx context.Context, req *core.RerankRequest) (*core.RerankResponse, error) {
	if req == nil {
		return nil, core.NewInvalidRequestError("rerank request is required", nil)
	}
	forwardReq := *req
	forwardReq.Model = p.transformModel(req.Model)
	var resp core.RerankResponse
	err := p.Do(ctx, llmclient.Request{
		Method:   http.MethodPost,
		Endpoint: "/rerank",
		Body:     &forwardReq,
