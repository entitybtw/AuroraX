...
// TestRouterSupports_PoolQualifiedSelector is a regression test for the bug
// where Router.Supports returned false for pool-qualified models like
// "jina-pool/jina-embeddings-v3" even when a pool member exposed the model.
// ResolveModel correctly identified the pool, but the follow-up lookup.Supports
// call then rejected the pool name because the model registry only indexes
// real provider names. The fix short-circuits Supports to true whenever the
// resolved selector points at a configured pool.
func TestRouterSupports_PoolQualifiedSelector(t *testing.T) {
	member := &mockProvider{name: "jina_1"}

	lookup := newMockLookup()
	lookup.addModel("jina_1/jina-embeddings-v3", member, "openai")
	...
	jinaPool, err := pool.NewPool(pool.Config{
		Name:     "jina-pool",
		Strategy: pool.StrategyRoundRobin,
		Members:  []pool.MemberConfig{{ProviderName: "jina_1"}},
	})
	if err != nil {
		t.Fatalf("pool.NewPool() error = %v", err)
	}
	if err := pools.Register(jinaPool); err != nil {
		t.Fatalf("pools.Register() error = %v", err)
	}
	...
	tests := []struct {
		...
		{"pool-qualified model exposed by member", "jina-pool/jina-embeddings-v3", true},
		{"provider-qualified model still works", "jina_1/jina-embeddings-v3", true},
		{"pool-qualified missing model", "jina-pool/does-not-exist", false},
		{"unknown pool name falls through", "ghost-pool/jina-embeddings-v3", false},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := router.Supports(tt.model); got != tt.want {
				t.Errorf("Supports(%q) = %v, want %v", tt.model, got, tt.want)
			}
		})
	}
}
...
func TestRouterEmbeddings_PoolSelectorMatchesProviderQualifiedJinaModel(t *testing.T) {
	...
	registry := newTestRegistryWithModels(registryModelEntry{
		...
		providerName: "jina_1",
		providerType: "openai",
		modelID:      "jina-ai/jina-embeddings-v3",
	})
	...
	if provider.lastEmbeddingReq == nil || provider.lastEmbeddingReq.Model != "jina-embeddings-v3" {
		t.Fatalf("expected upstream model jina-embeddings-v3, got %#v", provider.lastEmbeddingReq)
	}
	if provider.lastEmbeddingReq.Provider != "" {
		t.Fatalf("expected provider field stripped upstream, got %q", provider.lastEmbeddingReq.Provider)
	}
}
