...
// ResolvePricing returns pricing metadata with provenance for a model,
// trying the registry first and falling back to a reverse-index lookup
// via the model list. Returns nil if no pricing is available.
func (r *ModelRegistry) ResolvePricing(model, providerType string) *usage.PricingResult {
	provenance := r.buildPricingProvenance()
	...
	} else if meta := r.GetModelMetadata(model); meta != nil && meta.Pricing != nil {
		pricing = meta.Pricing
	} else if providerSelector != "" {
		if meta := r.ResolveMetadata(r.metadataProviderType(providerSelector), r.metadataModelID(model)); meta != nil && meta.Pricing != nil {
			pricing = meta.Pricing
		}
	}
	if pricing == nil {
	...
	return &usage.PricingResult{
		Pricing:    pricing,
		Provenance: provenance,
	}
}

// parseProvenanceTimestamp tries known timestamp layouts used by the model list
// and returns the parsed time. Returns zero time if none match.
// parseProvenanceTimestamp tries known timestamp layouts used by the model list.
// Timezone-aware layouts are tried first so that time.Parse does not silently
// drop a timezone offset by matching a tz-naive layout on a shorter prefix.
func parseProvenanceTimestamp(s string) time.Time {
	layouts := []string{
		time.RFC3339,
...
// buildPricingProvenance constructs provenance metadata from the current
// model list state. Safe for concurrent use (read lock held internally).
// Returns non-nil provenance even when the model list is not loaded.
func (r *ModelRegistry) buildPricingProvenance() *usage.PricingProvenance {
	r.mu.RLock()
	defer r.mu.RUnlock()

	p := &usage.PricingProvenance{
		Source:     "upstream_registry",
		ResolvedAt: time.Now().UTC(),
	}
	if r.modelList != nil {
		p.Version = strconv.Itoa(r.modelList.Version)
		if r.modelList.UpdatedAt != "" {
			if t := parseProvenanceTimestamp(r.modelList.UpdatedAt); !t.IsZero() {
				p.ResolvedAt = t
			}
		}
	}
