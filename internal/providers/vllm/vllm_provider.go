...
// fingerprint required by the OpenCode free tier. The SIDECAR suffix is
...
// targets OpenCode Zen, or an empty string otherwise. A provider-level
...
func New(cfg providers.ProviderConfig, opts providers.ProviderOptions) core.Provider {
	// Remember whether this provider targets OpenCode Zen (before any sidecar
	// rewrite) so the OAuth / uTLS / User-Agent treatment below still applies.
	isZen := strings.Contains(cfg.BaseURL, "opencode.ai/zen")

	// Route OpenCode Zen through the local Bun sidecar when configured. The
