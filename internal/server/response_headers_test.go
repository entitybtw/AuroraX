...
func TestHandleErrorWithResponseHeaders(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			provider:   "provider-ft-1",
			requested:  "provider-ft-1/ds-flash-free",
			wantActual: map[string]string{"X-Actual-Provider": "provider-ft-1", "X-Requested-Model": "provider-ft-1/ds-flash-free"},
		},
		{
		...
		},
		{
			...
			provider:   "provider-ft-1",
			requested:  "provider-ft-1/ds-flash-free",
			wantActual: map[string]string{"X-Actual-Provider": "provider-ft-1"},
		},
	}
