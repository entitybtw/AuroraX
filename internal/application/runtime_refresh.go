...
func (a *App) RefreshRuntime(ctx context.Context) (admin.RuntimeRefreshReport, error) {
	...
	if err := a.runRuntimeRefreshStep(&report, "providers_rebuilt", func() runtimeRefreshStepResult {
		if a.providers == nil {
			return runtimeRefreshStepResult{err: fmt.Errorf("provider runtime is unavailable")}
		}
		...
		if len(rawProviders) == 0 {
			return runtimeRefreshStepResult{
				...
				message: "no fresh provider config snapshot is available",
			}
		}
		count, err := a.providers.Rebuild(ctx, rawProviders, a.runtimeRawPools(), a.config, a.providers.Factory)
		if err != nil {
			return runtimeRefreshStepResult{
				status:  admin.RuntimeRefreshStatusFailed,
				message: "kept previous provider runtime",
				err:     err,
			}
		}
		registry = a.modelRegistry()
...
func (a *App) runtimeRawProviders() map[string]config.RawProviderConfig {
	...
	out := cloneRawProviderConfigs(a.rawProviders)
...
func (a *App) runtimeRawPools() map[string]config.RawPoolConfig {
	if a == nil {
		return nil
	}
	out := cloneRawPoolConfigs(a.rawPools)
	if a.poolOverrides == nil {
		return out
	}
	return a.poolOverrides.ApplyToRawPools(out)
}
