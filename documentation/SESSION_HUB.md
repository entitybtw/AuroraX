...
| `map_or_generate` | **(default / recommended)** Like `map`, but when the inbound header is absent a fresh value is generated instead of being skipped. Ideal for clients like the the CLI that only sometimes send a session header. |
...
> **Upstream requirements matter.** Some upstreams validate the exact shape of identity headers. The free-tier tier, for example, only accepts `x-opencode-session` of the form `ses_` + exactly 26 **hex** characters (and `x-opencode-request` as `msg_` + 26 hex). Use `charset: hex, length: 26` for those. Non-conforming rules are flagged in the dashboard and auto-corrected by the sidecar at request time.
...
| `POST` | `/admin/api/v1/sessionhub/providers/:name/ensure-opencode` | Idempotently merge the canonical upstream header rules (never overwrites existing rules) |
...
### One-click upstream rules

`POST /admin/api/v1/sessionhub/providers/:name/ensure-opencode` installs the canonical upstream header rules for a provider or pool:
...
upstream providers can also be configured from **Settings → Sidecar → Configure rules**, which opens an opt-in dialog calling the `ensure-opencode` endpoint above.
