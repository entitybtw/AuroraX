...
- **OpenCode sidecar** — bundled Bun sidecar that reproduces the OpenCode client fingerprint to unlock **OpenCode free-tier** models, with **multi-IP egress** (per-IP Go CONNECT proxies), a dedicated **Settings → Sidecar** tab (enable, port, tools injection, auth, retry, bind IPs), a one-click **Session Hub rule** dialog, and graceful fallback for paid/OAuth traffic (see [OpenCode Sidecar](#opencode-sidecar--free-tier-access) below).
...
- **Configurable charset** — generated values use `alphanumeric` (default), `hex`, or `digits`; some upstreams require a specific alphabet (e.g. OpenCode free tier needs `hex`)
...
- **One-click / preset OpenCode rules** — the Sidecar tab's **Quick setup** applies a preset that idempotently installs the canonical `x-opencode-*` rules (`POST /admin/api/v1/sessionhub/providers/:name/ensure-opencode`); existing rules are preserved, only missing ones are added, and non-safe values are flagged
...
## OpenCode Sidecar — Free-Tier Access

AuroraX can route **OpenCode Zen** (`opencode.ai/zen/v1`) and **opencode-go** traffic — including the **free tier** — through a bundled **Bun sidecar** that reproduces the official OpenCode client fingerprint. The sidecar runs inside the `runtime-sidecar` image variant and is managed from **Settings → Sidecar**.

> **⚠️ Use at your own risk.** The sidecar emulates the OpenCode client to reach the free tier. Upstream hardening can break it at any time, it may violate the provider's terms of service, and it relies on an actively maintained fingerprint. Review this section before enabling, and prefer a paid/API-key path for production workloads.

### Why a sidecar exists

OpenCode's Zen edge applies **layered** client verification. A request must pass **all** of these simultaneously:
...
| 3. Tool schema | Body must carry the **full 11-tool OpenCode schema** + `tool_choice: auto` | Zen rejects partial tool sets |
...
Then configure an OpenCode provider (type `vllm` or `opencode`) with:
...
| Inject tools | `AURORA_SIDECAR_INJECT_TOOLS` | `true` | Add the 11 OpenCode tools when absent |
...
- **Update the profile** — bump `BUN_VERSION` in `Dockerfile.builder` and the `User-Agent` / tool schema (`internal/providers/sidecarclient/sidecar/default-tools.json`) to match the latest OpenCode CLI.
...
- Free-tier availability depends on OpenCode's current policy and can change without notice.
...
AuroraX routes OpenCode Zen (`opencode.ai/zen/v1`) via the `vllm` provider type with optional multi-IP rotation, session-hub header mapping, and the Bun sidecar for free-tier access.
...
See [OpenCode Sidecar](#opencode-sidecar--free-tier-access) above. The sidecar handles both free and paid traffic and is the supported path for the free lineup.

### Free-tier model lineup

`mimo-v2.5-free`, `nemotron-3.5-lightning-free`, `nemotron-3-ultra-free`, `deepseek-v4-flash-free`, `muse-spark-1.3-contributor-free`, `muse-spark-1.2-contributor-free`, `ling-3.0-flash-fin-free` — availability tracks OpenCode's policy.
