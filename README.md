...
## OpenCode Zen — Known Limitations

AuroraX supports routing through the OpenCode Zen API (`opencode.ai/zen/v1`) using `vLLM` provider type with multi-IP rotation and session hub header mapping.

### What works

- **Paid models** (if your Zen account has credits) — full API compatibility
- **Pool rotation** — round-robin across multiple Zen API keys with distinct source IPs
...
- **User-Agent** — matches the real OpenCode CLI format: `opencode/<version>`

### What doesn't work (server-side limitation)

**Free-tier models return `FreeTierError`** — OpenCode's Zen API checks server-side whether the request originates from the actual OpenCode CLI application. This is **not** a header check — it is an authentication method check:

| Auth method | Free tier | Paid tier |
|-------------|-----------|-----------|
| OAuth token (from `opencode auth login opencode` device flow) | Allowed | Allowed (with credits) |
| API key (`sk-...` from zen dashboard) | **Blocked** (`FreeTierError`) | Allowed (with credits) |

The server verifies the Bearer token is an **OAuth access token** obtained through the device flow (`client_id: opencode-cli`, issuer `opencode.ai`), not a dashboard API key. No combination of headers (User-Agent, x-opencode-client, x-opencode-session, referer, etc.) can bypass this check.
...
- **User-Agent corrected** — pool `UserAgent` changed from `opencode-cli/1.18.31` to `opencode/1.18.31` to match the real OpenCode client format
