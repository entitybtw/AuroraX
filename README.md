...
- **One-click OpenCode rules** — the Sidecar tab can idempotently install the canonical `x-opencode-*` rules (`POST /admin/api/v1/sessionhub/providers/:name/ensure-opencode`); existing rules are preserved, only missing ones are added, and non-default values are flagged
